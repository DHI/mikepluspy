...
from NetTopologySuite.Geometries import LineString
...
class DataTableAccess:
	...
	def insert(self, table_name, muid, values=None):
		...
		if values is not None:
			for col in values:
				value_dict[col] = values[col]
		'''geom = GeoAPIHelper.CreateILineString(0.0, 0.1, 0.0, 0.1)'''
	...
	def is_database_open(self):
		if self._datatables == None:
			return False
		return self._datatables.DataSource != None and self._datatables.DataSource.DbConnection != None and self._datatables.DataSource.DbConnection.State == ConnectionState.Open